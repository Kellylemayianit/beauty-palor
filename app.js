 ...
 */
const WHATSAPP_NUMBER = "254786247132";
...
function onBookingSuccess(data, form, successBanner) {
  // 1. Format the date to a readable long-form string
  ...
  // 2. Format the time using the existing formatTime function
  const formattedTime = formatTime(data.time);

  // 3. Build the message array with the new brand name
  const message = [
    `✨ *New Booking — Denis Ng'ang'a Beauty & Spa*`,
    ``,
    `*Name:* ${data.name}`,
